...
int main(int argc, char* argv[])
{
    ...
    std::string sample_day = "20210813";
    ...
    std::string output_path = "D:\\chen1ting\\StockRawData\\kzzdata\\bin2csvOutput\\";
    //用户输入
    if (argc >= 5)
    {
        sample_day = argv[1];
        for (int i = 0; i < 3; i++)
            user_input_paths[i] = argv[i + 1];
    ...
    {
        std::string yesterday = "20210813";//*dates_it; // 
        ...
        for (auto it = ins_ids.begin(); it != ins_ids.end(); ++it)
        {
            ...
            if (MktMgr.CheckSimCorrectness() == false)
            {
                ...
                MktMgr.Trades2File(output_path);
            }
            else
