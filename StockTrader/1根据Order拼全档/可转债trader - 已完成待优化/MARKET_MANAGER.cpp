...
constexpr float MINGAP = 0.001;         //价格最小变动范围
constexpr double TRADEPB = 0.1;         //Trade Price Bound：连续竞价、收盘集合竞价的价格涨跌幅限制
constexpr double TEMPSTOP1 = 0.2;       //临停价格1：停30min
constexpr double TEMPSTOP2 = 0.3;       //临停价格2：停到14：57
...
void MarketManager::GetLstClosePrice(fs::path mktfilepath)
{
    ...
    Mkt.PriceBound[0] = round(Mkt.LstClose * (1 - TEMPSTOP2) * 1000.0) / 1000.0 + 0.00001;
    Mkt.PriceBound[1] = round(Mkt.LstClose * (1 - TEMPSTOP1) * 1000.0) / 1000.0 + 0.00001;
    Mkt.PriceBound[2] = round(Mkt.LstClose * (1 + TEMPSTOP1) * 1000.0) / 1000.0 - 0.00001;
    Mkt.PriceBound[3] = round(Mkt.LstClose * (1 + TEMPSTOP2) * 1000.0) / 1000.0 - 0.00001;
...
{
    //订单时间小于等于9:25，直接挂起
    while (Time1Bigger(NINE_TWENTY_FIVE_AM, current_ord->OrderTime) && ord_ptr < Mkt.OrdsCount)
    {
    ...
    DealCancelledOrder((current_ord - 1)->OrderIndex);
    CallAuction(OPENAUC);
    //PrintMktInfo();
    //订单时间小于14:57,连续竞价
    while (Time1Bigger(TWO_FIFTY_SEVEN_PM, current_ord->OrderTime) && ord_ptr < Mkt.OrdsCount)
    {
        //if (current_ord->OrderIndex == 4454851)
        //    std::cout << "blah";
        ...
        {//并且当前订单时间小于等于临停结束时间,直接挂起
            ...
            {
                DealCancelledOrder(current_ord ->OrderIndex);         //先处理该订单之前的所有单子
        ...
        {
            DealCancelledOrder(current_ord->OrderIndex);  //先撤单
            ContBid();  //再连续竞价
        }
    ...
    if (Mkt.StopTrade == true) //如果市场状态是临停,说明该订单是临停阶段下的单子,应挂起
    {
        ...
        DealCancelledOrder((txn_buffer + (Mkt.TxnCount - 1))->TradeIndex); //处理该单子之前的撤单
        CallAuction(STOPAUC);   //进行复牌集合竞价
        Mkt.StopTrade = false;  //跳出临停状态
    }
    while (ord_ptr < Mkt.OrdsCount)//将所有收盘集合竞价的单子挂起
    {
        HangOrd(*current_ord, current_ord->FunctionCode, true);
        current_ord++;
        ord_ptr++;
    }
    DealCancelledOrder((txn_buffer + (Mkt.TxnCount - 1))->TradeIndex); //处理所有剩余撤单
...
void MarketManager::DealCancelledOrder(long long idx)
{
    //已经读到最后一笔成交单了
    if (txn_ptr == Mkt.TxnCount)
        return;
    //成交单在current_order之前
...
//Utility Funcs
inline bool MarketManager::IsValidPrice(double Price){return (Mkt.Lower < Price&& Price < Mkt.Upper);}
inline void MarketManager::UpdateBound(double Price)
{
    Mkt.Upper = round(Price * (1 + TRADEPB) * 1000.0) / 1000.0 + MINGAP * 0.1;
    Mkt.Lower = round(Price * (1 - TRADEPB) * 1000.0) / 1000.0 - MINGAP * 0.1;
}
inline void MarketManager::UpdateBestOffer()
{
    ...
    {
        if (best_offer_pos->first < Mkt.Lower)
        {
            do
                best_offer_pos++;
            while (best_offer_pos != --offers_order_book.end() && best_offer_pos->first < Mkt.Lower);
            Mkt.BestOffer = best_offer_pos->first;
        }
        else
        {
            do
                best_offer_pos--;
            while (best_offer_pos != --offers_order_book.begin() && best_offer_pos->first > Mkt.Lower);
...
inline void MarketManager::UpdateBestBid()
{
    ...
    {
        if (best_bid_pos->first > Mkt.Upper)
        {
            do
                best_bid_pos--;
            while (best_bid_pos != bids_order_book.begin() && best_bid_pos->first > Mkt.Upper);
            Mkt.BestBid = best_bid_pos->first;
        }
        else
        {
            do
                best_bid_pos++;
            while (best_bid_pos != bids_order_book.end() && best_bid_pos->first < Mkt.Upper);
            Mkt.BestBid = (--best_bid_pos)->first;
        }
...
bool MarketManager::CheckStopTrade()
{
    ...
    if (Mkt.NewestPrice <= Mkt.PriceBound[0] ||
        (Mkt.FirstTimeFallStop == true && Mkt.NewestPrice <= Mkt.PriceBound[1]) ||
        (Mkt.FirstTimeRiseStop == true && Mkt.NewestPrice >= Mkt.PriceBound[2]) ||
        Mkt.NewestPrice >= Mkt.PriceBound[3])
    {
        Mkt.StopTrade = true;
        memcpy_s(Mkt.StopTradeUntil, 13, (current_ord + 1)->OrderTime, 13);
        if(Mkt.FirstTimeFallStop == true && Mkt.NewestPrice <= Mkt.PriceBound[1])
            Mkt.FirstTimeFallStop = false;
        else if (Mkt.FirstTimeRiseStop == true && Mkt.NewestPrice >= Mkt.PriceBound[2])
...
void MarketManager::Trades2File(std::string output)
{
    std::ofstream trade2f;
    trade2f.open(output + std::string(order_buffer->InstrumentID) + "SimTrade.csv");
...
void MarketManager::Levels2File(std::string output, std::string filename)
{
    std::ofstream level2f;
    level2f.open(output + filename + "Levels.csv");
    level2f << "卖盘" << std::endl;
